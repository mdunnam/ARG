...
- **Content ledger** — a DynamoDB table (`somnatek-content-ledger`) tracks release state for each artifact: `drafted` → `scheduled` → `released` → `discovered` → `superseded` → `removed`.
...
3. Update `somnatek-content-ledger` record to `released`.
...
**Current implementation status (April 30, 2026):**
- Stage 1 — ✅ Live
...
The email responder Lambda (`lambda/email-responder/index.js`) is live and classifies inbound emails to `records@somnatek.org` by keyword content. This is the primary mechanism for Stage 3–4 escalation without requiring Discord infrastructure.
...
The phone line (404-671-9774) routes through Amazon Connect. The inbound fax plays the morse-code audio artifact (Morse for “413” at 15 WPM). This is a passive Stage 2 signal — callers hear it but receive no interactive response.
